        ...
        Self: Sized,
    {
        ...
        match token {
            ...
            CasteTag::MentalAttributeRate {
                ..
            } => token.parse_labeled_array(&values, |attribute,
            [improvement_cost,
            decay_rate_unused,
            decay_rate_rusty,
            decay_rate_demotion,]|CasteTag::MentalAttributeRate { attribute, improvement_cost, decay_rate_unused, decay_rate_rusty, decay_rate_demotion }),
            CasteTag::Milkable {
                ..
            } => token.parse_vector_with_tail(&values, |material,
            frequency,|CasteTag::Milkable { material, frequency }),
            CasteTag::ModValue { .. } => token.parse_single(&values, |value|CasteTag::ModValue { value }),
            CasteTag::Name {
            ..
            } => token.parse_array(&values, |[singular,
                plural,
                adjective,]|CasteTag::Name { singular, plural, adjective }),
            CasteTag::NaturalSkill { .. } => token.parse_labeled_array(&values, |skill, [level]|CasteTag::NaturalSkill { skill, level }),
            CasteTag::OdorLevel { .. } => token.parse_single(&values, |odor_level|CasteTag::OdorLevel { odor_level }),
            CasteTag::OdorString { .. } => token.parse_single(&values, |odor_string|CasteTag::OdorString { odor_string }),
            CasteTag::Orientation {
                ..
            } => token.parse_labeled_array(&values, |caste,
            [disinterested_chance,
            casual_chance,
            strong_chance,]|CasteTag::Orientation { caste, disinterested_chance, casual_chance, strong_chance }),
            CasteTag::PenetratePower { .. } => token.parse_single(&values, |penetrate_power| CasteTag::PenetratePower { penetrate_power }),
            CasteTag::Personality {
                ..
            } => token.parse_labeled_array(&values, |personality_trait,
            [low,
            median,
            high,]|CasteTag::Personality { personality_trait, low, median, high }),
            CasteTag::PetValue { .. } => token.parse_single(&values, |pet_value| CasteTag::PetValue { pet_value }),
            CasteTag::PetValueDivisor { .. } => token.parse_single(&values, |divisor| CasteTag::PetValueDivisor { divisor }),
            CasteTag::PhysicalAttributeCapPercentage {
                ..
            } => token.parse_labeled_array(&values, |attribute,
            [percentage,]|CasteTag::PhysicalAttributeCapPercentage { attribute, percentage }),
            CasteTag::PhysicalAttributeRange { .. } => token.parse_labeled_array(&values, |attribute, ranges| CasteTag::PhysicalAttributeRange { attribute, ranges }),
            ...
            decay_rate_unused,
            decay_rate_rusty,
            decay_rate_demotion,]| Self::PhysicalAttributeRate { attribute, improvement_cost, decay_rate_unused, decay_rate_rusty, decay_rate_demotion }),
            CasteTag::PlusBodyPartGroup { ..
            } => token.parse_vector(&values, |selector| CasteTag::PlusBodyPartGroup { selector }),
            CasteTag::PopulationRatio { .. } => token.parse_single(&values, |pop_ratio|CasteTag::PopulationRatio { pop_ratio }),
            CasteTag::ProfessionName {
            ..
            } => token.parse_array(&values, |[profession,
                singular,
                plural,]| CasteTag::ProfessionName { profession, singular, plural }),
            CasteTag::ProneToRage { .. } => token.parse_single(&values, |rage_chance|CasteTag::ProneToRage { rage_chance }),
            CasteTag::Pus { .. } => token.parse_vector_with_tail(&values, |material, state |CasteTag::Pus { material, state }),
            CasteTag::RelativeSize { ..
            } => token.parse_vector_with_tail(&values, |
                selector,
                relative_size,|CasteTag::RelativeSize {
                    selector,
                    relative_size, }),
            CasteTag::Remains { .. } => token.parse_array(&values, |[singular, plural]|CasteTag::Remains { singular, plural }),
            CasteTag::RemainsColor { .. } => token.parse_single(&values, |remains_color|CasteTag::RemainsColor { remains_color }),
            CasteTag::RetractIntoBodyPart {
                ..
            } => token.parse_array(&values, |[body_part_selector,
            body_part,
            second_person,
            third_person,
            second_person_cancel,
            third_person_cancel,]| CasteTag::RetractIntoBodyPart { body_part_selector, body_part, second_person, third_person, second_person_cancel, third_person_cancel }),
            CasteTag::RootAround {
                ..
            } => token.parse_array(&values, |[body_part,
            body_part_selection,
            second_person_verb,
            third_person_verb,]| CasteTag::RootAround { body_part_selector: vec![body_part, body_part_selection], second_person_verb, third_person_verb }),
            CasteTag::Secretion { ..
            } => token.parse_vector(&values, |mut args| {
                    // Safety check: Material(1+) + State(1) + Selector(2) + Tissue(1) + Trigger(1) = 6 min
                    if args.len() < 6 {
                        tracing::warn!("Secretion tag missing required arguments: {}/6 {:?}", args.len(), args);
                        return None;
                    }
            ...
            CasteTag::SenseCreatureClass {
                ..
            } => token.parse_array(&values, |[creature_class,
            tile,
            foreground,
            background,
            brightness,]| CasteTag::SenseCreatureClass { creature_class, tile, foreground: foreground.parse::<u32>().unwrap_or_default(), background: background.parse::<u32>().unwrap_or_default(), brightness: brightness.parse::<u32>().unwrap_or_default()       }),
            CasteTag::SetBodyPartGroup { ..
            } =>  token.parse_vector(&values, |body_part_selector|CasteTag::SetBodyPartGroup { body_part_selector }),
            CasteTag::SkillLearnRate { .. } => token.parse_labeled_array(&values, |skill, [rate]|CasteTag::SkillLearnRate { skill, rate }),
            CasteTag::SkillLearnRates { ..  } => token.parse_single(&values, |rate|CasteTag::SkillLearnRates { rate }),
            CasteTag::SkillRate {
                ..
            } => token.parse_labeled_array(&values, |skill,
            [improvement_rate,
            decay_rate_unused,
            decay_rate_rusty,
            decay_rate_demotion,]|CasteTag::SkillRate { skill, improvement_rate, decay_rate_unused, decay_rate_rusty, decay_rate_demotion }),
            CasteTag::SkillRates {
                ..
            } => token.parse_array(&values, |[improvement_rate,
            decay_rate_unused,
            decay_rate_rusty,
            decay_rate_demotion,]| CasteTag::SkillRates { improvement_rate, decay_rate_unused, decay_rate_rusty, decay_rate_demotion }),
            CasteTag::SkillRustRate {
                ..
            } => token.parse_labeled_array(&values, |skill,
            [decay_rate_unused,
            decay_rate_rusty,
            decay_rate_demotion,]| CasteTag::SkillRustRate { skill, decay_rate_unused, decay_rate_rusty, decay_rate_demotion }),
            CasteTag::SkillRustRates {
                ..
            } => token.parse_array(&values, |[decay_rate_unused,
                decay_rate_rusty,
                decay_rate_demotion,]| CasteTag::SkillRustRates { decay_rate_unused, decay_rate_rusty, decay_rate_demotion }),
            CasteTag::SlainSpeech { .. } => token.parse_single(&values, |speech_file|CasteTag::SlainSpeech { speech_file }),
            CasteTag::SoldierTile { .. } => token.parse_single(&values, |tile| CasteTag::SoldierTile { tile }),
            CasteTag::SoldierAltTile { .. } => token.parse_single(&values, |tile| CasteTag::SoldierAltTile { tile }),
            CasteTag::Sound { ..
            } => {
            ...
            if values.len() < 2 {
                tracing::warn!("Not enough args for SpecificFood: {}/2: {:?}",
                    &values.len(),
                    &values);
                return None; }
            ...
            CasteTag::SyndromeDilutionFactor {
                ..
            } => token.parse_labeled_array(&values, |syndrome,
                [percentage]|CasteTag::SyndromeDilutionFactor { syndrome, percentage }),
            CasteTag::Tendons { ..
            } => token.parse_vector_with_tail(&values, |
                material,
                healing_rate,|CasteTag::Tendons { material, healing_rate }),
            ...
            CasteTag::TissueLayer {
                ..
                    ...
                    token.parse_vector(&values, |mut args: Vec<String>| {
                        ...
                        let tissue = if args.len() >= 3 { args.remove(2) } else { String::new() };
                        let body_part = if args.len() >= 2 { args.remove(1) } else { String::new() };
                        let body_part_selection = if !args.is_empty() { args.remove(0) } else { String::new() };
            ...
            ..
                ...
                body_part,
                tissue,]| CasteTag::TissueLayerUnder { body_part_selector, body_part, tissue }),
            CasteTag::TradeCapacity { .. } => token.parse_single(&values, |capacity| CasteTag::TradeCapacity { capacity }),
            CasteTag::VerminBite {
                ..
                    ...
                    let chance = iter.next()
                        .and_then(|s| token.parse_value(&s))
                        .unwrap_or(0);
            ...
            CasteTag::ViewRange { .. } => token.parse_single(&values, |view_range| CasteTag::ViewRange { view_range }),
            CasteTag::VisionArc {
            ..
            } => token.parse_array(&values, |[binocular,
                non_binocular,]|CasteTag::VisionArc { binocular, non_binocular }),
            CasteTag::Webber { .. } => token.parse_vector(&values, |material| CasteTag::Webber { material }),
        }
    }
}
